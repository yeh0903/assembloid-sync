...
```sh
#!/bin/sh
# bin/orgpipe (Git Bash) - lives in bin/ because the repo-root name is taken
# by the orgpipe/ package directory (a file cannot share its parent's name)
"C:/ProgramData/anaconda3/python.exe" "Z:/Joseph/orgpipe/bin/pipeline.py" "$@"
```

Run: `chmod +x bin/orgpipe`. Git Bash invocations use `./bin/orgpipe`; cmd/PowerShell
use `orgpipe.bat` at the repo root (no collision - different filename). Putting
`Z:\Joseph\orgpipein` on PATH makes bare `orgpipe` work everywhere.

- [ ] **Step 4: Verify status and the analyze gate**

Run: `./bin/orgpipe status`
Expected: a table of non-B3 datasets; `250528_B2_003` shows `done*` for denoise/fiji/suite2p (smoke runs from tasks 7–8), curated `-`

Run: `./bin/orgpipe analyze 250528_B2_003`
Expected: `REFUSING 250528_B2_003: not curated yet (mtime gate)...`, exit 1

Run: `./bin/orgpipe analyze 250528_B2_003 --assume-curated`
...
```bash
./bin/orgpipe run 250528_B2_003 --smoke --force --no-gui
```
...
```bash
./bin/orgpipe run 250528_B2_003 --smoke --force --no-gui
./bin/orgpipe analyze 250528_B2_003 --assume-curated --force
./bin/orgpipe status
```
...
```bash
./bin/orgpipe run 250528_B2_003 --force
```
...
```bash
./bin/orgpipe analyze 250528_B2_003
```
...
## Execution findings (amendments applied during implementation)

- **`if __name__ == "__main__":` guard on every `bin/run_*.py`** — Windows spawn-based
  multiprocessing re-executes the entry module in each worker; an unguarded
  `entry.main(...)` recursed into full pipeline invocations (observed as a process storm
  during Task 7). All entry-script templates above now carry the guard.
- **`write_denoised` passes `metadata=None`** — tifffile's default per-write "shaped"
  metadata splits ragged chunk sizes into separate series, so `imread` silently dropped
  the tail chunk. With `metadata=None`, output is byte-identical to a single-shot write.
- **Fiji stage** — `open()` replaced by an explicit Bio-Formats Importer call (BigTIFF
  otherwise pops a blocking dialog in GUI mode) and completion is detected via a
  DONE_SENTINEL.txt written by the macro + process-tree kill (the JVM outlives
  `System.exit` after a Bio-Formats import). Verified pixel-identical to the manual
  output on all 5400 frames, display-range drift 0.
- **caiman 1.11.4 ignores the flat `cnn_thr` and `min_fitness_raw` keys** (logs
  "provided toplevel key ... was unused"). The manual notebook passed the identical flat
  dict, so manual runs had the same behavior — keeping the keys preserves fidelity;
  they are inert in both paths.
